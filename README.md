...
未署名exeは、環境によって Windows Code Integrity / Application Control でブロックされることがあります。コード署名は発行元を示し、未署名扱いによるブロックや警告を減らすための候補です。インストーラー整備は、配置場所、ショートカット、アンインストール手順を固定し、古いexeの取り違えを減らすための候補です。配布先フォルダを固定するだけでも、家族利用では手順の迷いを減らせます。
...
`v1.1` 候補のExplorer連携版はApps配布先へ反映済みです。実画像615件のフォルダで、画像場所、現在フォルダ、保存先、画像パスコピー、フォルダパスコピーが動作することを確認済みです。
